...
├── src/
│   ├── main.c               # 初期化 + メインループ骨格 (各モジュールは weak フック)
...
`buttons_*` / `usb_link_*` / `ddc_tally_*` は後続タスクで実装されるモジュールのフックで、
`main.c` では `__attribute__((weak))` 宣言のみを置いている（未実装時は no-op）。
...
I/O に依存しないロジック（`config.h` の定数、`get_controller_id()`、今後追加される
debounce/tally decode など）はホストの `gcc` でネイティブビルド・実行してテストする。
