...
// スロット(MODULE_BUS_COUNT本のI2Cバス、いずれもアドレスは MODULE_I2C_ADDR)の
// STATE(0x00) をポーリングし、共有状態配列を更新する。**1回の呼び出しで1バスだけ**進む
// ラウンドロビンで、全スロット一巡には MODULE_BUS_COUNT 回の呼び出しが必要。1回の
// 呼び出しのブロック時間を1トランザクション分に抑え、USB(tud_task)を待たせないため。
// 1モジュールのタイムアウト/NACK/バス異常はそのモジュールのみ present=false として
